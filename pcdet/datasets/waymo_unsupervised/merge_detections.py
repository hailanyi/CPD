    ...
    for root, dirs, files in os.walk(path, topdown=True):
        for name in files:
            ...
            for k in range(17, 21):
                if 'epoch_'+str(k) in root:
                    flag = True
            if name[-3:] == 'pkl' and root[-3:]=='val' in root and ('result_merged.pkl' != name) and flag:
...
class DetectionsMerger():
    def __init__(self,config_path,det_path):
    ...
    def merge(self):
        out_path = os.path.join(det_path,'result_merged.pkl')
...
if __name__ == '__main__':
    config_path = '/home/asc01/projects/OpenProjects/OpenPCDetV2/tools/cfgs/dataset_configs/waymo/waymo_pretrain_dataset.yaml'
    det_path = '/home/asc01/projects/OpenProjects/OpenPCDetV2/output/models/waymo_unsupervised/voxel_rcnn_cproto_center_si/default/eval'

    merger = DetectionsMerger(config_path,det_path)
    merger.merge()