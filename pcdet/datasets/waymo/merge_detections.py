...
def read_pickles(path, cfg):
    ...
    for root, dirs, files in os.walk(path, topdown=True):
        for name in files:
            flag = True
            for k in range(580, 590):
                if str(k) in root:
                    flag = False
            if name[-3:] == 'pkl' and root[-3:]=='ain' and ('result_merged.pkl' != name) and flag:
...
if __name__ == '__main__':
    ...
    det_path = '/home/asc01/projects/OpenProjects/OpenPCDetV2/output/models/waymo/center_point/centerpoint/default/eval'
