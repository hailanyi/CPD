...
class WaymoDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(self, info_path, save_path, save_ssd_path=None,used_classes=None, split='train', sampled_interval=10,
                                    processed_data_tag=None):
        gt_path_name = Path('pcdet_gt_database_%s_cp' % split)

        if save_ssd_path is None:
            database_save_path = save_path / gt_path_name
        else:
            database_save_path = save_ssd_path / gt_path_name

        db_info_save_path = save_path / ('pcdet_waymo_dbinfos_%s_cp.pkl' % split)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default='/home/projects/wh/OpenProjects/OpenPCDet/tools/cfgs/one_frame/dataset_configs/waymo_dataset.yaml', help='specify the config of dataset')
    ...
    if args.func == 'create_waymo_infos':
        ...
        dataset_cfg = EasyDict(yaml.safe_load(open(args.cfg_file))) # EasyDict 像访问属性一样访问dict里的变量
