...
from pcdet.datasets.data_utils import save_pp_score
from pcdet.datasets.data_utils import save_outline_box, optimize_outline_box
import multiprocessing
from functools import partial

class WaymoPretrainDataset(DatasetTemplate):
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            if os.path.exists(self.data_path / sequence_name / ('%s_outline_opti.pkl' % sequence_name)):
                info_path = self.data_path / sequence_name / ('%s_outline_opti.pkl' % sequence_name)
            else:
    ...
    def get_frame(self, index):
        ...
        for i in range(index-num_frames+1, index+1):
            ...
            if i>=0:
                ...
                if this_seq_idx==sequence_name:
                    ...
                    this_points[:, -1] = i - (index-num_frames)
        ...
        if self.dataset_cfg.current_label_method == 'unlabeled' and self.training:
            ...
            '''
            points, outline_box, outline_ids, outline_cls = remove_past(points,
                                                                        outline_box,
                                                                        outline_ids,
                                                                        outline_cls)
            '''
    ...
    def create_track_groundtruth_database(self, info_path, save_path, used_classes=None, split='train'):
        ...
        for cls_name in used_classes:
            ...
            for k in range(0, len(infos)):
                ...
                outline_ids = info['outline_ids']
                outline_cls = info['outline_cls']
                outline_score = info['outline_score']
                thresh = self.dataset_cfg.OptimizeConfig.ScoreThresh
                outline_box = outline_box[outline_score>thresh]
                outline_ids = outline_ids[outline_score>thresh]
                outline_cls = outline_cls[outline_score>thresh]
    ...
    def optimize_outline_box(self, root_path):
        sample_sequence_file_list = [ os.path.splitext(os.path.basename(sequence_file))[0]
                                      for sequence_file in self.sample_sequence_list]

        process_single_sequence = partial(
            optimize_outline_box,
            root_path=str(root_path),
            config=self.dataset_cfg,
        )
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    print('---------------Start to optimize outline box---------------')
    waymo_infos_train = dataset.optimize_outline_box(root_path=save_path / processed_data_tag)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default='waymo_pretrain_dataset.yaml', help='specify the config of dataset')
    ...
    if args.func == 'create_waymo_infos':
        ...
        ROOT_DIR = '/home/asc01/data/waymo/perception'
