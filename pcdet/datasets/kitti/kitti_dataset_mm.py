...
class KittiDatasetMM(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, test_iter=0):
        ...
        """
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger, test_iter=test_iter
        )
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            lidar_mm_path = Path(self.root_split_path) / self.dataset_cfg.MM_PATH
            lidar_mm_path.mkdir(parents=True, exist_ok=True)
            this_lidar_mm_path = lidar_mm_path / ('%s.npy' % sample_idx)
            this_calib = self.get_calib(sample_idx)
            image = self.get_image(sample_idx)

            new_points = self.print_im2lidar(this_calib, image, points,
                                         self.dataset_cfg.COL_RES,
                                         self.dataset_cfg.ROW_RES)
            new_points[:,4:]/=255
            np.save(str(this_lidar_mm_path), new_points.astype(np.float16))
            if has_label:
    ...
    def print_im2lidar(self, calib, image, points, dx=1, dy=9):
        pts_rect = calib.lidar_to_rect(points[:, 0:3])
        fov_flag = self.get_fov_flag(pts_rect, image.shape, calib)
        points = points[fov_flag]

        pts_rect = calib.lidar_to_rect(points[:, 0:3])
        pts_img, pts_rect_depth = calib.rect_to_img(pts_rect)

        new_points = np.zeros(shape=(points.shape[0], 4 + dx * dy * 3))
        new_points[:, 0:4] = points[:, 0:4]

        for i, im_coord in enumerate(pts_img):
            x_coord = int(im_coord[0])
            y_coord = int(im_coord[1])

            min_x = x_coord - dx // 2
            max_x = x_coord + dx // 2 + 1
            min_y = y_coord - dy // 2
            max_y = y_coord + dy // 2 + 1
            ...
            features = image[min_y: max_y, min_x: max_x]
            features = features.reshape(-1)

            new_points[i, 4:4 + features.shape[0]] = features
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        database_save_path = Path(self.root_path) / ('gt_database_mm_TWISE3' if split == 'train' else ('gt_database_%s_mm_TWISE3' % split))
        db_info_save_path = Path(self.root_path) / ('kitti_dbinfos_%s_mm_TWISE3.pkl' % split)
...
if __name__ == '__main__':
    ...
    dataset_cfg = EasyDict(yaml.load(open("/home/asc01/projects/OpenProjects/OpenPCDet/tools/cfgs/dataset_configs/kitti/kitti_dataset.yaml")))
    ROOT_DIR = Path('/home/asc01/data/kitti/detection')
    create_kitti_infos(
