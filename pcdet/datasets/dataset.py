...
from .augmentor.X_transform import X_TRAIN
from .augmentor.X_transform import X_TEST
from .processor.data_processor import DataProcessor
from .processor.point_feature_encoder import PointFeatureEncoder

class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, is_source=True, root_path=None, logger=None, test_iter=0,
                 da_train=False):
        super().__init__()
        self.num_data_frames = dataset_cfg.get('NUM_FRAMES', 1)
        self.merge_frame = dataset_cfg.get('MERGE_FRAME', True)
        ...
        if self.training:
            self.stages = self.dataset_cfg.get('STAGES', 2)
        else:
            self.stages = self.dataset_cfg.get('STAGES_T', 2)

        self.num_frames = self.stages
        ...
        self.data_augmentor = DataAugmentor(
            ...
            num_frames=1, dataset_cfg=dataset_cfg,
        ) if self.training else None

        test_aug_cfg = self.dataset_cfg.get('TEST_AUGMENTOR', None)

        if test_aug_cfg is not None:
            self.test_augmentor = TestAugmentor(test_aug_cfg[test_iter], self.class_names, logger=self.logger,
                                            num_frames=self.stages)
        else:
            self.test_augmentor = None

        self.data_processor = DataProcessor(
            self.dataset_cfg.DATA_PROCESSOR, point_cloud_range=self.point_cloud_range, training=self.training,
            stages=self.stages, num_point_features=self.point_feature_encoder.num_point_features
        )




        x_trans_train_cfg = self.dataset_cfg.get('X_TRAIN', None)
        if x_trans_train_cfg is not None:
            self.x_trans_train = X_TRAIN(x_trans_train_cfg, stages=self.stages)
        else:
            self.x_trans_train = None

        x_trans_test_cfg = self.dataset_cfg.get('X_TEST', None)
        if x_trans_test_cfg is not None:
            self.x_trans_test = X_TEST(x_trans_test_cfg, stages=self.stages)
        else:
            self.x_trans_test = None
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            if self.x_trans_train is not None:
                data_dict = self.x_trans_train.input_transform(
                    data_dict={
                        **data_dict,
                    }
                )
            ...
            if self.x_trans_test is not None:
                data_dict = self.x_trans_test.input_transform(
                    data_dict={
                        **data_dict,
                    }
                )
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            for i in range(self.stages):
                if i == 0:
                    ...
                    stage_id = str(i)
                if 'gt_boxes'+stage_id in data_dict:
                    data_dict['gt_boxes'+stage_id] = data_dict['gt_boxes'+stage_id][selected]
                    gt_classes = np.array([self.class_names.index(n) + 1 for n in data_dict['gt_names']], dtype=np.int32)
                    gt_boxes = np.concatenate((data_dict['gt_boxes'+stage_id], gt_classes.reshape(-1, 1).astype(np.float32)), axis=1)
                    data_dict['gt_boxes'+stage_id] = gt_boxes
        ...
        for i in range(self.stages):
            if i ==0:
                ...
                stage_id = str(i)
            if 'mm' in data_dict:
                data_dict['points_mm'+stage_id] = data_dict['points'+stage_id][data_dict['points'+stage_id][:, -1] == 1]
                data_dict['points'+stage_id] = data_dict['points'+stage_id][data_dict['points'+stage_id][:, -1] == 2]
        ...
        for key, val in data_dict.items():
            try:
                if key in voxel_key_dict:
                    ...
                    max_gt = max([len(x) for x in val])
                    batch_gt_score = np.zeros((batch_size, max_gt), dtype=np.float32)
                    for k in range(batch_size):
                        batch_gt_score[k, :val[k].__len__()] = val[k]
                    ret[key] = batch_gt_score
                elif key == 'proto_group_id':
                    max_gt = max([len(x) for x in val])
                    batch_gt_proto = np.ones((batch_size, max_gt), dtype=np.float32)*(-1)
                    for k in range(batch_size):
                        batch_gt_proto[k, :val[k].__len__()] = val[k]
                    ret[key] = batch_gt_proto
                else:
...
class SeqDatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.class_names = class_names
        self.logger = logger
        self.root_path = root_path if root_path is not None else Path(os.path.join(self.dataset_cfg.ROOT.ROOT_PATH, self.dataset_cfg.DATA_PATH))
        self.logger = logger
        if self.dataset_cfg is None or class_names is None:
            return
        ...
        self.data_processor = DataProcessor(
            self.dataset_cfg.DATA_PROCESSOR, point_cloud_range=self.point_cloud_range, training=self.training,
            stages=1, num_point_features=self.point_feature_encoder.num_point_features
        )
