...
class C_PROTO_SI():
    ...
    def compute_raw_proto(self):
        ...
        if os.path.exists(output_raw_proto_path):
            with open(output_raw_proto_path, 'rb') as f:
                infos = pkl.load(f)
            return infos
        ...
        tracker = Tracker3D(box_type='OpenPCDet', config=self.dataset_cfg.GeneratorConfig)
        ...
        for i in range(len(outline_infos)):
            ...
            _, outline_ids = tracker.tracking(bbs_3D=outline_box, scores=outline_score, pose=pose, timestamp=i)

            outline_infos[i]['outline_ids'] = outline_ids
