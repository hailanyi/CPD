...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict, ind=''):
        ...
        Returns:
            batch_dict:
                ...
                rcnn_cls_labels: (B, M)
        """
        batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, additional_data = self.sample_rois_for_rcnn(
    ...
    def sample_rois_for_rcnn(self, batch_dict, ind=''):
        ...
        if 'css_score' in batch_dict:
            css_score = batch_dict['css_score']
            batch_roi_css_score = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        if 'outline_cls_mask' in batch_dict:
            outline_cls_mask = batch_dict['outline_cls_mask']
            batch_roi_cls_mask = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        if 'outline_reg_mask' in batch_dict:
            outline_reg_mask = batch_dict['outline_reg_mask']
            batch_roi_reg_mask = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        ...
        for index in range(batch_size):
            ...
            if 'css_score' in batch_dict:
                cur_css_score1 = css_score[index]
                cur_css_score2 = cur_css_score1[:k+1]

                batch_roi_css_score[index] = cur_css_score2[gt_assignment[sampled_inds]]

            if 'outline_cls_mask' in batch_dict:
                cur_outline_cls_mask1 = outline_cls_mask[index]
                cur_outline_cls_mask2 = cur_outline_cls_mask1[:k+1]
                batch_roi_cls_mask[index] = cur_outline_cls_mask2[gt_assignment[sampled_inds]]
            if 'outline_reg_mask' in batch_dict:
                cur_outline_reg_mask1 = outline_reg_mask[index]
                cur_outline_reg_mask2 = cur_outline_reg_mask1[:k + 1]
                batch_roi_reg_mask[index] = cur_outline_reg_mask2[gt_assignment[sampled_inds]]

        if 'css_score' in batch_dict:
            additional_data['css_score'] = batch_roi_css_score
        if 'outline_cls_mask' in batch_dict:
            additional_data['outline_cls_mask'] = batch_roi_cls_mask
        if 'outline_reg_mask' in batch_dict:
            additional_data['outline_reg_mask'] = batch_roi_reg_mask

        return batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, additional_data
