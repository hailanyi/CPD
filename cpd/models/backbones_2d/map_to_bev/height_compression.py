...
class HeightCompression(nn.Module):
    def __init__(self, model_cfg, num_frames, voxel_size=None, point_cloud_range=None):
        super().__init__()
        self.num_frames=num_frames
        self.model_cfg = model_cfg
        self.num_bev_features = self.model_cfg.NUM_BEV_FEATURES
        self.x_trans = X_TRAIN()
    ...
    def forward(self, batch_dict):
        ...
        for i in range(stages):
            ...
            encoded_spconv_tensor = batch_dict['encoded_spconv_tensor'+stage_id]
            spatial_features = encoded_spconv_tensor.dense()
            N, C, D, H, W = spatial_features.shape
            spatial_features = spatial_features.view(N, C * D, H, W)

            batch_dict['spatial_features'+stage_id] = spatial_features

            if i==0:
                all_feat.append(spatial_features)
            elif 'transform_param' in batch_dict and i>0 and self.model_cfg.get('ALIGN', False):
        ...
        if 'transform_param' in batch_dict and self.model_cfg.get('ALIGN', False):
            all_feat = torch.stack(all_feat)
            if self.model_cfg.get('ALIGN_METHOD', 'first') == 'first':
                batch_dict['spatial_features'] = batch_dict['spatial_features']
            elif self.model_cfg.get('ALIGN_METHOD', 'first') == 'max':
                final_feat = all_feat.max(0)[0]
                batch_dict['spatial_features'] = final_feat
            elif self.model_cfg.get('ALIGN_METHOD', 'first') == 'mean':
                final_feat = all_feat.mean(0)
                batch_dict['spatial_features'] = final_feat
            elif self.model_cfg.get('ALIGN_METHOD', 'first') == 'weighted_max':
                w1 = self.model_cfg.get('W1', 0.9)
                w2 = self.model_cfg.get('W2', 0.1)
                final_feat_max = all_feat.max(0)[0]
                first_feat = batch_dict['spatial_features']
                batch_dict['spatial_features'] = w1*first_feat+w2*final_feat_max

        '''
        if batch_dict['frame_id'][0]=='000366':
            print(batch_dict['frame_id'])
            spatial_features = batch_dict['spatial_features'].clone() #B,C,W,H
            np.save('feat_good.npy', spatial_features.max(1)[0].cpu().numpy())
            input()
        '''
...
class BEVPool(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        if 'transform_param' in batch_dict and self.model_cfg.get('ALIGN', False):
            ...
            if self.model_cfg.get('ALIGN_METHOD', 'first') == 'max':
                final_feat = all_feat.max(0)[0]
                batch_dict['spatial_features'] = final_feat
            elif self.model_cfg.get('ALIGN_METHOD', 'first') == 'mean':
                final_feat = all_feat.mean(0)
