...
class X_TRAIN(object):
    def __init__(self, augmentor_configs=None, stages=1):
        self.stages = stages
    ...
    def get_params(self):
        transform_param = np.zeros(shape=(self.stages, len(self.aug_config_list)))
        for s in range(self.stages):
            for i, config in enumerate(self.aug_config_list):
                if config.NAME == 'world_rotation':
                    transform_param[s][i] = np.random.uniform(config.WORLD_ROT_ANGLE[0], config.WORLD_ROT_ANGLE[1])
                if config.NAME == 'world_flip':
                    transform_param[s][i] = np.random.choice([0, 1], replace=False, p=[0.5, 0.5])
                if config.NAME == 'world_scaling':
                    transform_param[s][i] = np.random.uniform(config.WORLD_SCALE_RANGE[0],config.WORLD_SCALE_RANGE[1])
            if self.augmentor_configs.get('FIXED', False):
                break

        if self.augmentor_configs.get('FIXED', False):
            for s in range(1, self.stages):
                transform_param[s] = transform_param[0]
    ...
    def input_transform(self, data_dict):
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
        ...
        for i in range(self.stages):
            ...
            data_dict['points'+stage_id] = transformed_data['points']
            data_dict['gt_boxes'+stage_id] = transformed_data['boxes']
...
class X_TEST(object):
    def __init__(self, augmentor_configs, stages=1):
        self.stages = stages
        self.data_augmentor_queue = []
        self.test_back_queue = []
        self.aug_config_list = augmentor_configs if isinstance(augmentor_configs, list) \
    ...
    def get_params(self):
        #self.stages = len(self.aug_config_list[0].WORLD_ROT_ANGLE)
        transform_param = np.zeros(shape=(self.stages, len(self.aug_config_list)))
        for s in range(self.stages):
            for i, config in enumerate(self.aug_config_list):
                if config.NAME == 'world_rotation':
