...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, num_frames, logger=None, dataset_cfg=None):
    ...
    def sample_with_fixed_number_previous(self, class_name, sample_group):
        """
        Args:
            class_name:
            sample_group:
        ...
        for this_sample in sampled_dict:
            if self.dataset_cfg.current_label_method in this_sample['labeling_method_dict']:
                new_dict.append(this_sample)
    ...
    def sample_with_fixed_number(self, class_name, sample_group):
        ...
        while len(new_dict) < sample_num:

            this_dict = self.sample_with_fixed_number_previous(class_name, sample_group)
            new_dict+=this_dict
...
class DADataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene_multi(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if 'road_plane' in data_dict:                # mv height
                obj_points[:, 2] -= mv_height[idx]
        ...
        if self.num_frames > 1:
            ...
            for i in range(1, self.num_frames):
                ...
                if 'gt_names' + str(-i) not in data_dict:
                    ...
                    pre_gt_boxes_mask = np.array([n in self.class_names for n in data_dict['gt_names' + str(-i)]],
                                                 dtype=np.bool_)
                    pre_gt_boxes = data_dict['gt_boxes' + str(-i)][pre_gt_boxes_mask]
                    pre_gt_names = data_dict['gt_names' + str(-i)][pre_gt_boxes_mask]
                    pre_gt_idx = np.array(data_dict['ob_idx' + str(-i)])[pre_gt_boxes_mask]
                ...
                for idx, info in enumerate(total_valid_sampled_dict):
                    ...
                    if 'box3d_lidar' + str(-i) in info:
                        ...
                        pre_obj_points, pre_box3d_lidar = self.registration(pose, pre_pose, pre_obj_points,
                                                                            pre_box3d_lidar)
            ...
            if 'gt_tracklets' in data_dict:
                data_dict["gt_tracklets"] = np.concatenate([data_dict["gt_tracklets"], gt_tracklets], axis=0)
                data_dict["num_bbs_in_tracklets"] = np.concatenate(
                    [data_dict["num_bbs_in_tracklets"], num_bbs_in_tracks], axis=0)
