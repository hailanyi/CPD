...
class PointFeatureEncoder(object):
    def __init__(self, config, point_cloud_range=None, stages=1):
        super().__init__()
        self.stages=stages
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.stages):
            if i == 0:
                ...
                stage_id = str(i)
            data_dict['points'+stage_id], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
                data_dict['points'+stage_id]
            )

            if 'mm' in data_dict:
                data_dict['points_mm'+stage_id], use_lead_xyz = getattr(self, self.point_encoding_config.encoding_type)(
                    data_dict['points_mm'+stage_id]
                )
